...
var _ = ginkgo.Describe("[Load Balancer] EWMA", func() {
	...
	ginkgo.It("does not fail requests", func() {
		...
		assert.Equal(ginkgo.GinkgoT(), algorithm, "ewma") // want "error-first: assert error before making other assertions" "expected-actual: need to reverse actual and expected values"
	})
})
