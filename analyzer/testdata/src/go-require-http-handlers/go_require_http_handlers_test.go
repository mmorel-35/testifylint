...
func TestServer_Require(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, err = w.Write(data)
		if !assert.NoError(t, err) {
			assert.FailNow(t, err.Error()) // want "go-require: do not use assert\\.FailNow in http handlers" "fail-now: use t\\.Fatal or t\\.Fatalf instead"
		}
	}))
	defer ts.Close()
...
func (s *ServerSuite) ServeHTTP(w http.ResponseWriter, _ *http.Request) {
	...
	_, err = w.Write(data)
	if !s.NoError(err) {
		s.FailNow(err.Error()) // want "go-require: do not use s\\.FailNow in http handlers" "fail-now: use t\\.Fatal or t\\.Fatalf instead"
	}
}
